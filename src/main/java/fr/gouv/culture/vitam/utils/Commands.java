...
public class Commands {
	...
	public static final void addFormatIdentification(Element root, String basename, File fic, 
		...
		try {
			...
			if (identityFound && finalFormat.getMimeType().equals("message/rfc822")) {
				// email special task
				Element special = EmlExtract.extractInfoEmail(fic, basename, argument, config);
				metadata.detach();
				Element meta = (Element) special.selectSingleNode("metadata");
				if (meta != null) {
					root.add(meta.detach());
				}
				Element keyw = (Element) special.selectSingleNode("keywords");
				if (keyw != null) {
